...
export default function WrapperModal({ className = '', children, open, title, icon, onClose }) {
    ...
    return (
        <div className={`${open ? 'open' : ''} wrapper-modal fixed left-[0] top-[0] bottom-[0] right-[0] min-w-full min-h-screen `}>
            <dialog open={ open } className={`z-[2] max-w-[518px] min-w-[518px] rounded shadow fixed top-[50%] translate-y-[-50%] ${className}`}>
            ...
            <div className="overlay fixed left-0 top-0 bottom-0 right-0  w-full h-full z-[9999]"></div>
        </div>
    );
}